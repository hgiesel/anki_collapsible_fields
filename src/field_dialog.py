...
def load_collapsible_option(self, idx):
    ...
    self.form.collapsible.setChecked(fld[collapse_by_default_keyword] if collapse_by_default_keyword in fld else False)

def init_field_dialog():
    # setupSignals is called before executing QDialog
    FieldDialog.setupSignals = wrap(FieldDialog.setupSignals, init_collapsible_option, 'after')

    FieldDialog.saveField = wrap(FieldDialog.saveField, save_collapsible_option, 'after')
    FieldDialog.loadField = wrap(FieldDialog.loadField, load_collapsible_option, 'after')
